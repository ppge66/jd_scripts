...
async function helpFriends() {
  for (let code of $.newShareCodes) {
    if (!code) continue;
    if (UserShareCodes == code) continue;
        ...
        try {
          if (safeGet(data)) {
            ...
            if (oc(() => data.data.result.taskVos)) {
              ...
              let strSharedcode = `${oc(() => data.data.result.taskVos[0].assistTaskDetailVo.taskToken)}`;
        ...
        try {
          if (safeGet(data)) {
            ...
            if (taskId === -1) {
              ...
              await doTask(
                oc(() => data.data.result.taskVos[0].shoppingActivityVos[0].taskToken),
                22,
                1,
              ); // 领取任务
              ...
              for (let vo of nc(
                oc(() => data.data.result.taskVos.filter((vo) => ![19, 25, 15, 21].includes(vo.taskType))),
                [],
              )) {
                ...
                for (let i = vo.times; i < vo.maxTimes; i++) {
                  ...
                  if (vo.taskType === 13) {
                    await doTask(
                      oc(() => vo.simpleRecordInfoVo.taskToken),
                      oc(() => vo.taskId),
                    );
                  } else if (vo.taskType === 8) {
                    await doTask(
                      oc(() => vo.productInfoVos[i].taskToken),
                      oc(() => vo.taskId),
                      1,
                    );
                    await $.wait(1000 * 10);
                    ...
                    await doTask(
                      oc(() => vo.shoppingActivityVos[0].taskToken),
                      oc(() => vo.taskId),
                      1,
                    );
                    await $.wait(1000 * 10);
                    ...
                    await doTask(
                      oc(() => vo.threeMealInfoVos[0].taskToken),
                      oc(() => vo.taskId),
                    );
        ...
        if (safeGet(data)) {
          ...
          if (oc(() => data.data.bizCode) === 0) {
            if (oc(() => data.data.result.produceScore))
              console.log(
                `任务完成成功，获得：${nc(
                  oc(() => data.data.result.produceScore),
                  '未知',
                )}能量`,
              );
            else
