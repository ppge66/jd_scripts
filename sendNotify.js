...
async function sendNotify(text, desp, params = {}, author = '\n\n本通知 By ccwav Mod', strsummary = '') {
  ...
  if (process.env.NOTIFY_FILTERBYFILE) {
    let no_notify = process.env.NOTIFY_FILTERBYFILE.split('&');
  ...
  try {
    ...
    // 变量开关
    let Use_serverNotify = true;
    let Use_pushPlusNotify = true;
    let Use_BarkNotify = true;
    let Use_tgBotNotify = true;
    let Use_ddBotNotify = true;
    let Use_qywxBotNotify = true;
    let Use_qywxamNotify = true;
    let Use_iGotNotify = true;
    let Use_gobotNotify = true;
    let Use_pushPlushxtripNotify = true;
    let Use_WxPusher = true;
    let strtext = text;
    let strdesp = desp;
    ...
    if (process.env.NOTIFY_SKIP_TEXT && desp) {
      ...
      if (Notify_SkipText.length > 0) {
        for (let Templ in Notify_SkipText) {
    ...
    if (text.indexOf('cookie已失效') != -1 || desp.indexOf('重新登录获取') != -1 || text == 'Ninja 运行通知') {
      if (Notify_CKTask) {
        ...
        await exec(Notify_CKTask, (error, stdout, stderr) => {
    ...
    if (process.env.NOTIFY_AUTOCHECKCK == 'true') {
      if (text.indexOf('cookie已失效') != -1 || desp.indexOf('重新登录获取') != -1) {
        ...
        let llHaderror = false;

        if (strPtPin) {
          let temptest = await getEnvByPtPin(strdecPtPin);
          if (temptest) {
          ...
          if (strTitle == strCustomTempArr[0]) {
            ...
            if (strCustomTempArr.length > 2) {
              ...
              for (let Tempk = 2; Tempk < strCustomTempArr.length; Tempk++) {
                let strTrmp = strCustomTempArr[Tempk];
                switch (strTrmp) {
                  case 'Server酱':
        ...
        for (let i = 0; i < envs.length; i++) {
          ...
          if ($.Remark) {
            ...
            try {
              ...
              if (text == '京东资产变动' || text == '京东资产变动#2' || text == '京东资产变动#3' || text == '京东资产变动#4') {
                let Tempinfo = getQLinfo(cookie, envs[i].created, envs[i].timestamp, envs[i].remarks);
                if (Tempinfo) {
                  $.Remark += Tempinfo;
                }
              }
  ...
  if (boolneedUpdate) {
    let str = JSON.stringify(TempCK, null, 2);
    fs.writeFile(strCKFile, str, (err) => {
      if (err) {
        console.log(err);
...
function getuuid(strRemark, PtPin) {
  let strTempuuid = '';
  if (strRemark) {
    let Tempindex = strRemark.indexOf('@@');
    if (Tempindex != -1) {
      console.log(PtPin + ': 检测到NVJDC的一对一格式,瑞思拜~!');
      let TempRemarkList = strRemark.split('@@');
...
function getQLinfo(strCK, intcreated, strTimestamp, strRemark) {
  let strCheckCK = strCK.match(/pt_key=([^; ]+)(?=;?)/) && strCK.match(/pt_key=([^; ]+)(?=;?)/)[1];
  let strPtPin = decodeURIComponent(strCK.match(/pt_pin=([^; ]+)(?=;?)/) && strCK.match(/pt_pin=([^; ]+)(?=;?)/)[1]);
  let strReturn = '';
  if (strCheckCK.substring(0, 4) == 'AAJh') {
    let DateCreated = new Date(intcreated);
    let DateTimestamp = new Date(strTimestamp);
    ...
    // 过期时间
    let UseDay = Math.ceil((DateToday.getTime() - DateCreated.getTime()) / 86400000);
    let LogoutDay = 30 - Math.ceil((DateToday.getTime() - DateTimestamp.getTime()) / 86400000);
    if (LogoutDay < 1) {
...
async function sendNotifybyWxPucher(text, desp, PtPin, author = '\n\n本通知 By ccwav Mod', strsummary = '') {
  try {
    ...
    if (WP_APP_TOKEN_ONE) {
      let tempEnv = await getEnvByPtPin(PtPin);
      if (tempEnv) {
        ...
        if (Uid) {
          ...
          if (text == '京东资产变动') {
            try {
              ...
              let Tempinfo = getQLinfo(cookie, tempEnv.created, tempEnv.timestamp, tempEnv.remarks);
              if (Tempinfo) {
...
async function GetPtPin(text) {
  try {
    ...
    if (TempList.length > 1) {
      let strNickName = TempList[TempList.length - 1];
      let strPtPin = '';
...
function Env(t, s) {
    return new(class {
        ...
        isSurge() {
            return 'undefined' !== typeof $httpClient && typeof $loon === 'undefined';
        }
        isLoon() {
        ...
        lodash_set(t, s, e) {
            return Object(t) !== t ? t : (Array.isArray(s) || (s = s.toString().match(/[^.[\]]+/g) || []), (s.slice(0, -1).reduce((t, e, i) => (Object(t[e]) === t[e] ? t[e] : (t[e] = Math.abs(s[i + 1]) >> 0 == Number(s[i + 1]) ? [] : {})), t)[s[s.length - 1]] = e), t);
        }
        getdata(t) {
            let s = this.getval(t);
            if (/^@/.test(t)) {
                const[, e, i] = /^@(.*?)\.(.*?)$/.exec(t);
                const o = e ? this.getval(e) : '';
                if (o)
                    try {
        ...
        setdata(t, s) {
            ...
            if (/^@/.test(s)) {
                const[, i, o] = /^@(.*?)\.(.*?)$/.exec(s);
                const h = this.getval(i);
                const a = i ? (h === 'null' ? null : h || '{}') : '{}';
                try {
        ...
        time(t) {
            ...
            /(y+)/.test(t) && (t = t.replace(RegExp.$1, (String(new Date().getFullYear())).substr(4 - RegExp.$1.length)));
            for (let e in s)
                new RegExp('(' + e + ')').test(t) && (t = t.replace(RegExp.$1, RegExp.$1.length == 1 ? s[e] : ('00' + s[e]).substr((String(s[e])).length)));
            return t;
        }
        msg(s = t, e = '', i = '', o) {
            const h = (t) => !t || (!this.isLoon() && this.isSurge()) ? t : typeof t === 'string' ? this.isLoon() ? t : this.isQuanX() ? {
                'open-url': t
            }
             : void 0 : typeof t === 'object' && (t['open-url'] || t['media-url']) ? this.isLoon() ? t['open-url'] : this.isQuanX() ? t : void 0 : void 0;
